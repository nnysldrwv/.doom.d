...
** Fonts
Maple Mono NF CN for monospace (CJK + Nerd Font all-in-one), LXGW WenKai Screen for variable pitch.
#+begin_src emacs-lisp
;; (defconst my/font-main "Maple Mono NF CN")

;; (setq doom-font (font-spec :family my/font-main :size 16)
;;       doom-big-font (font-spec :family my/font-main :size 26)
;;       doom-variable-pitch-font (font-spec :family my/font-main :size 18))

;; (defun my/apply-maple-fonts (&optional _frame)
;;   "Use Maple Mono NF CN everywhere."
...
;;                         (font-spec :family my/font-main)
;;                         nil 'prepend))

;;     ;; 再补一层 unicode 范围
;;     (set-fontset-font t '(#x2e80 . #x9fff)
;;                       (font-spec :family my/font-main)
;;                       nil 'prepend)

;;     (message "[font-maple] %s" my/font-main)))

;; (add-hook 'after-setting-font-hook #'my/apply-maple-fonts)
;; (add-hook 'window-setup-hook #'my/apply-maple-fonts)

;; (setq word-wrap-by-category t)

;; (after! org
;;   ;; 你如果想 Org 正文也统一 Maple，就不要开 variable-pitch-mode
;;   ;; 如果你仍想保留 Org 的“正文风格”，可以打开，但字体仍然是 Maple
;;   ;; (add-hook 'org-mode-hook #'variable-pitch-mode)

;;   (custom-set-faces!
;;     '(org-document-title :inherit (bold) :height 1.20)
;;     '(org-level-1 :inherit (bold) :height 1.12)
;;     '(org-level-2 :inherit (bold) :height 1.08)
;;     '(org-level-3 :inherit (bold) :height 1.04)
...
;; (message "[fonts-final] all=%s" my/font-main)

#+end_src

#+begin_src emacs-lisp
;; ;; 抄的ljg的配置，但如果symbol-font配置的是Sarasa Mono SC,中文就会退化成Jhenghei
;; ;; Doom 自带的字体变量配置
;; ;; 中文Maple Mono, 英文Roboto Mono
...
;;       doom-variable-pitch-font (font-spec :family "LXGW WenKai Screen" :weight 'extra-bold))
...
#+begin_src emacs-lisp
;; 问题很清楚。doom-symbol-font 只影响 symbol 范围（数学符号、箭头等），不覆盖 CJK
;; 汉字文字。汉字属于 han 脚本，Emacs 从默认 fontset 里捡了 Microsoft JhengHei。

;; 你需要的配置是：拉丁用 Roboto Mono，汉字用 Sarasa Mono SC。在
;; after-setting-font-hook 里加 han / cjk-misc / kana / hangul 脚本的映射：

  ;; Doom 自带的字体变量配置
  ;; 拉丁用 Roboto Mono, 汉字用 Sarasa Mono SC
  ;; (setq doom-font (font-spec :family "Roboto Mono" :size 22)
  (setq doom-font (font-spec :family "Ubuntu Mono" :size 22)
        doom-big-font (font-spec :family "Ubuntu Mono" :size 36)
        doom-serif-font doom-font
        doom-symbol-font (font-spec :family "Sarasa Mono SC")
        doom-variable-pitch-font (font-spec :family "Ubuntu Mono" :weight
  'extra-bold))
  (setq use-default-font-for-symbols nil)
...
** Display: spacious-padding / olivetti / ultra-scroll / pangu-spacing / org-appear
#+begin_src emacs-lisp
(after! org
...
** Delete file link in Org (replaces org-download-delete)
#+begin_src emacs-lisp
