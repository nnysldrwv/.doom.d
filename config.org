...
;; 高速增量拼写检查 (Windows 需要 enchant: scoop install enchant)
(package! jinx)
...
;; - Windows 安装依赖: scoop install enchant
...
(defun my/denote-journal--insert-weather-async (target-file)
  "异步获取天气并写入 TARGET-FILE 末尾, 避免新建 journal 时卡顿。"
...
(defun my/denote-journal-auto-weather ()
  "新建 journal 文件时异步追加天气, 不阻塞 capture 流程。"
  (when (and (buffer-file-name)
