...
(defun my/beancount-record-buy ()
  ...
  (let* ((date (my/beancount--read-date))
         ...
         (amount-default (format "%.2f" (* (my/beancount--number price)
                                            (my/beancount--number shares))))
         (amount (my/beancount--read-number-string "成交金额: " amount-default))
...
(defun my/beancount-record-sell ()
  ...
  (let* ((date (my/beancount--read-date))
         ...
         (amount-default (format "%.2f" (* (my/beancount--number price)
                                            (my/beancount--number shares))))
         (amount (my/beancount--read-number-string "成交金额: " amount-default))
         (fee (my/beancount--read-number-string "手续费/税费: " "0")))
    (my/beancount--append-entry
     (format "%s * \"卖出 %s (%s)\"\n  fund_code: \"%s\"\n  fund_name: \"%s\"\n  asset_category: \"%s\"\n  trade_price: \"%s CNY\"\n  trade_amount: \"%s CNY\"\n  %s  -%s %s {} @ %s CNY\n%s  Equity:Investment:Funding\n"
             date name code code name category price amount
