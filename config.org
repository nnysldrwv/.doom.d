...
(defun my/beancount--parse-balance-batch-line (line line-number)
  "Parse LINE at LINE-NUMBER into a balance posting plist."
  (let ((trimmed (string-trim line)))
    (unless (or (string-empty-p trimmed)
                (string-prefix-p "#" trimmed)
                (string-prefix-p ";" trimmed))
      (unless (string-match "\\`\\([^[:space:],，]+\\)[[:space:],，]+\\(.+\\)\\'" trimmed)
        (user-error "Line %d should be: CODE SHARES" line-number))
      (let* ((code (match-string 1 trimmed))
             (shares (my/beancount--normalize-number (match-string 2 trimmed)))
             (instrument (my/beancount--holding-instrument code))
             (account (plist-get instrument :account))
...
(defun my/beancount--balance-batch-build (date text)
  "Build a Beancount balance snapshot for DATE from pasted TEXT."
  (let (postings)
    (cl-loop for line in (split-string text "\n")
             for line-number from 1
             for parsed = (my/beancount--parse-balance-batch-line line line-number)
             when parsed
  ...
  (let* ((date (my/beancount--read-date))
         ...
         (counter-posting (cond
                            ...
                            ((string= out-target "股票账户")
                             (format "  Assets:Investment:Broker:Cash  %s CNY\n" amount))
