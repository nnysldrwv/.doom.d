...
;; - 正式交易用下面的 `my/beancount-record-*` 命令直接入账；org-capture 只做信息不完整时的暂存。
...
(defun my/beancount-record-sell ()
  ...
  (let* ((date (my/beancount--read-date))
    ...
    (my/beancount--append-entry
     (format "%s * \"卖出 %s (%s)\"\n  fund_code: \"%s\"\n  fund_name: \"%s\"\n  asset_category: \"%s\"\n  trade_price: \"%s CNY\"\n  trade_amount: \"%.2f CNY\"\n  %s  -%s %s {} @ %s CNY\n%s  Equity:Investment:Funding\n"
             date name code code name category price amount
             account shares commodity price
             (if (> fee 0)
                 (format "  Expenses:Investment:Fees  %.4f CNY\n" fee)
               ""))
     date code)))
...
(defun my/beancount-record-balance ()
  ...
  (let* ((date (my/beancount--read-date "校验日期: "))
         ...
         (commodity (plist-get fund :commodity))
         (account (plist-get fund :account))
         (shares (my/beancount--read-number-string "持有份额: ")))
    (my/beancount--append-entry
...
(defun my/beancount-rebuild-prices-history ()
  ...
  (let* ((script (expand-file-name "investment/scripts/update_prices.py" org-directory))
         (default-directory (expand-file-name "investment/" org-directory)))
    (compilation-start
     (format "python \"%s\" history" script)
     nil
     (lambda (_) "*beancount rebuild prices history*"))))

(map! :leader
      (:prefix ("n i" . "investment")
       :desc "Beancount buy"        "b" #'my/beancount-record-buy
       :desc "Beancount sell"       "s" #'my/beancount-record-sell
       :desc "Beancount price"      "p" #'my/beancount-record-price
       :desc "Beancount balance"    "B" #'my/beancount-record-balance
       :desc "Update prices latest" "u" #'my/beancount-update-prices-latest
       :desc "Rebuild price history" "H" #'my/beancount-rebuild-prices-history
       :desc "Open ledger"          "o" #'my/open-beancount-main
       :desc "Check ledger"         "c" #'my/beancount-check-main
       :desc "Fava"                 "f" #'my/beancount-fava-main))
#+end_src
